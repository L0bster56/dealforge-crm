...
class ChangePasswordUseCase:
    """
    UseCase: изменение пароля пользователя.

    Ответственность:
        - Проверка сложности нового пароля
        - Проверка, что новый пароль отличается от старого
        - Проверка старого пароля через hasher
    ...
    async def execute(self, cmd: ChangePasswordCommand):
        """
        Выполняет сценарий смены пароля пользователя.

        Args:
            cmd (ChangePasswordCommand): Команда смены пароля

        Raises:
            WeakPasswordError: если новый пароль слишком слабый
            SamePasswordError: если новый пароль совпадает со старым
            InvalidPasswordError: если старый пароль неверный
        """

        if not self.password_spec.is_satisfied_by(cmd.new_password):
            raise WeakPasswordError("new password is too weak")

        if not self.password_diff_spec.is_satisfied_by(
            (cmd.old_password, cmd.new_password)
        ):
        ...
        async with self.uow as uow:
            ...
            self.user.change_password(
                self.hasher.hash(cmd.new_password)
            )
